...
export default function DashboardPage() {
      ...
      if (isTeamMember) {
        ...
        return {
          ...
          tasks: (proj.tasks || []).filter(
            (t) => t.assigned_member === user.email,
          ),
        };
      }
      return proj;
  ...
  filteredProjects.forEach((proj) => {
    ...
    if (proj.dead_line) {
      ...
      const daysDiff = Math.ceil(
        (projDate.getTime() - today.getTime()) / (1000 * 3600 * 24),
      );
    ...
    (proj.members || []).forEach((m) => {
      if (!memberProductivity[m])
        memberProductivity[m] = { total: 0, completed: 0 };
    });
    ...
    tasks.forEach((t) => {
      ...
      if (t.assigned_member) {
        if (!memberProductivity[t.assigned_member])
          memberProductivity[t.assigned_member] = { total: 0, completed: 0 };
        memberProductivity[t.assigned_member].total++;
        if (t.status === "Completed")
          memberProductivity[t.assigned_member].completed++;
      }
    });
  });
  ...
  if (isTeamMember && user?.email) {
    allActivities = allActivities.filter((a) =>
      a.message.toLowerCase().includes(user.email.toLowerCase()),
    );
  }
  ...
  const statusOptions = {
    ...
    plotOptions: {
      bar: { borderRadius: 6, distributed: true, columnWidth: "50%" },
    },
    xaxis: {
  ...
  const memberEntries = Object.entries(memberProductivity)
  ...
  const productivitySeries = [
    ...
    {
      name: "Pending",
      data: memberEntries.map((e) => e[1].total - e[1].completed),
    },
  ];
  ...
  const trendSeries = [
    { name: "Velocity", data: [5, 12, 25, 40, 58, 72, completedTasks || 85] },
  ];
    ...
    <div className="h-full flex flex-col font-sans p-2 md:p-6 overflow-y-auto bg-background/50">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-8 shrink-0">
        {[
          ...
          },
          {
            ...
            color: overdueTasks > 0 ? "text-red-500" : "text-text-muted",
            bg: overdueTasks > 0 ? "bg-red-500/10" : "bg-card-bg",
            border:
              overdueTasks > 0 ? "border-red-500/20" : "border-card-border",
          },
        ].map((kpi, i) => (
          <div
            key={i}
            className={`rounded-2xl p-5 border shadow-sm backdrop-blur-md transition-transform hover:-translate-y-1 ${kpi.bg} ${kpi.border}`}
          >
            <div
              className={`w-10 h-10 rounded-xl mb-4 flex items-center justify-center bg-card-bg shadow-sm border border-card-border ${kpi.color}`}
            >
              <svg
                className="w-5 h-5"
                fill="none"
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
              ...
              {memberEntries.map((e) => {
                ...
                return (
                  <div key={e[0]}>
                    ...
                    <progress
                      className="progress progress-primary w-full bg-background border border-card-border"
                      value={pct}
                      max="100"
                    ></progress>
                  </div>
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-3">
              {highPriorityTasks.length === 0 ? (
                ...
                highPriorityTasks.slice(0, 4).map((t, i) => (
                  <div
                    key={i}
                    className="bg-card-bg border border-red-500/30 rounded-xl p-3 flex justify-between items-center shadow-sm"
                  >
                    <span className="text-xs font-bold text-foreground truncate mr-2">
                      {t.title}
                    </span>
                    <span className="text-[9px] px-1.5 py-0.5 rounded bg-red-500 text-white font-bold uppercase shrink-0">
                      High
                    </span>
                  </div>
                ))
          ...
          <div className="bg-card-bg border border-card-border rounded-2xl shadow-sm p-6">
            ...
            <div className="space-y-3">
              {upcomingDeadlines.length === 0 ? (
                <p className="text-[11px] text-text-muted font-semibold">
                  No deadlines within 72 hours.
                </p>
              ) : (
                upcomingDeadlines.slice(0, 4).map((d, i) => (
                  <div
                    key={i}
                    className="bg-background border border-card-border rounded-xl p-3 flex justify-between items-center"
                  >
                    <span className="text-xs font-bold text-foreground truncate mr-2">
                      {d.title}
                    </span>
                    <span
                      className={`text-[10px] font-black uppercase shrink-0 ${d.days === 0 ? "text-red-500 animate-pulse" : "text-yellow-500"}`}
                    >
          ...
          <div className="bg-card-bg border border-card-border rounded-2xl shadow-sm p-6">
            <h3 className="text-xs font-bold text-foreground uppercase tracking-wider mb-6">
              Recent Activity
            </h3>
                  ...
                  return (
                    <li key={act.id + index}>
                      ...
                      <div className="timeline-middle">
                        <div
                          className={`w-6 h-6 rounded-full bg-background border border-card-border flex items-center justify-center ${colorClass}`}
                        >
                          <svg
                            className="w-3 h-3"
                            fill="none"
